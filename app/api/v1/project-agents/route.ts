...
export async function GET(req: NextRequest) {
  try {
    ...
    const { items, total } = repo.listProjectAgents(
      { projectId: sp.get("projectId") ?? undefined, agentId: sp.get("agentId") ?? undefined },
      { page, pageSize }
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const relation = service.attachAgentToProject(body.projectId, body.agentId);
