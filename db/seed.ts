 ...
 * - 数据与前端 lib/mock-data/seed.ts 完全同构（同一确定性随机种子 20260908、同一频次），
 *   保证 Mock/Real 两模式统计口径可对比。
 * - Schema 变更永远不经过本文件（只走 drizzle migration）。
 */
...
/** 幂等 upsert helper（drizzle 泛型链限制，此处按演示 seed 工具放宽类型） */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function seedRows<T extends { id: string }>(table: any, rows: T[]) {
...
const seedDefinitions: Array<SeedDefinition & { createdAt: string; updatedAt: string }> = [
  { id: "skill-web-search", type: "skill", name: "网页搜索", description: "实时检索网页与资讯", lifecycle: "active", createdAt: new Date(now - 88 * DAY).toISOString(), updatedAt: new Date(now - 88 * DAY).toISOString() },
  { id: "skill-doc-summary", type: "skill", name: "文档摘要", description: "长文档要点提炼", lifecycle: "active", createdAt: new Date(now - 85 * DAY).toISOString(), updatedAt: new Date(now - 85 * DAY).toISOString() },
  { id: "skill-data-viz", type: "skill", name: "数据可视化建议", description: "图表选型与口径建议", lifecycle: "active", createdAt: new Date(now - 82 * DAY).toISOString(), updatedAt: new Date(now - 82 * DAY).toISOString() },
  { id: "skill-copywriting", type: "skill", name: "文案生成", description: "营销文案与社媒短句", lifecycle: "active", createdAt: new Date(now - 79 * DAY).toISOString(), updatedAt: new Date(now - 79 * DAY).toISOString() },
  { id: "memory-long-term", type: "memory", name: "长期记忆", description: "跨会话记住关键事实", lifecycle: "active", createdAt: new Date(now - 76 * DAY).toISOString(), updatedAt: new Date(now - 76 * DAY).toISOString() },
  { id: "memory-semantic", type: "memory", name: "语义记忆", description: "按语义检索历史知识", lifecycle: "active", createdAt: new Date(now - 73 * DAY).toISOString(), updatedAt: new Date(now - 73 * DAY).toISOString() },
  { id: "memory-episodic", type: "memory", name: "情景记忆", description: "记录任务执行历史", lifecycle: "active", createdAt: new Date(now - 70 * DAY).toISOString(), updatedAt: new Date(now - 70 * DAY).toISOString() },
  { id: "rule-tone", type: "rule", name: "语气规范", description: "输出语气克制专业", lifecycle: "active", createdAt: new Date(now - 67 * DAY).toISOString(), updatedAt: new Date(now - 67 * DAY).toISOString() },
  { id: "rule-citation", type: "rule", name: "引用可追溯", description: "事实必须附来源", lifecycle: "active", createdAt: new Date(now - 64 * DAY).toISOString(), updatedAt: new Date(now - 64 * DAY).toISOString() },
  { id: "rule-privacy", type: "rule", name: "脱敏输出", description: "敏感字段自动脱敏", lifecycle: "active", createdAt: new Date(now - 61 * DAY).toISOString(), updatedAt: new Date(now - 61 * DAY).toISOString() },
  { id: "rule-escalation", type: "rule", name: "升级规则", description: "风险工单转人工", lifecycle: "active", createdAt: new Date(now - 58 * DAY).toISOString(), updatedAt: new Date(now - 58 * DAY).toISOString() },
  { id: "rule-severity", type: "rule", name: "严重级别标注", description: "异常必须标注级别", lifecycle: "active", createdAt: new Date(now - 55 * DAY).toISOString(), updatedAt: new Date(now - 55 * DAY).toISOString() },
  { id: "tool-slack", type: "tool", name: "Slack 通知", description: "向频道发送消息", lifecycle: "active", createdAt: new Date(now - 52 * DAY).toISOString(), updatedAt: new Date(now - 52 * DAY).toISOString() },
  { id: "tool-crm", type: "tool", name: "CRM 查询", description: "读取客户与工单", lifecycle: "active", createdAt: new Date(now - 49 * DAY).toISOString(), updatedAt: new Date(now - 49 * DAY).toISOString() },
  { id: "tool-log-query", type: "tool", name: "日志查询", description: "检索服务日志", lifecycle: "active", createdAt: new Date(now - 46 * DAY).toISOString(), updatedAt: new Date(now - 46 * DAY).toISOString() },
  { id: "tool-db-reader", type: "tool", name: "数据库只读查询", description: "对数据源执行只读查询", lifecycle: "active", createdAt: new Date(now - 43 * DAY).toISOString(), updatedAt: new Date(now - 43 * DAY).toISOString() },
  { id: "skill-rss-digest", type: "skill", name: "RSS 订阅汇总", description: "定期汇总订阅源更新（已由网页搜索替代）", lifecycle: "archived", createdAt: new Date(now - 90 * DAY).toISOString(), updatedAt: new Date(now - 20 * DAY).toISOString() },
  { id: "tool-legacy-http", type: "tool", name: "遗留 HTTP 回调", description: "旧版回调通道，已停止维护", lifecycle: "archived", createdAt: new Date(now - 95 * DAY).toISOString(), updatedAt: new Date(now - 25 * DAY).toISOString() },
];

const seedAssemblies: Array<{ id: string; agentId: string; capabilityId: string; enabled: boolean; createdAt: string }> = [
...
const seedProjects = [
  { id: "project-web", name: "官网改版", description: "官网内容生成、竞品调研与文案产出", status: "active", createdAt: new Date(now - 42 * DAY).toISOString(), updatedAt: new Date(now - 6 * HOUR).toISOString() },
  { id: "project-support", name: "客服提效", description: "工单分诊、话术优化与客户数据洞察", status: "active", createdAt: new Date(now - 30 * DAY).toISOString(), updatedAt: new Date(now - 2 * HOUR).toISOString() },
  { id: "project-data", name: "数据分析平台", description: "日志分析、指标监控与图表体系建设", status: "active", createdAt: new Date(now - 18 * DAY).toISOString(), updatedAt: new Date(now - 1 * DAY).toISOString() },
];

const seedProjectAgentRows = [
...
export function runSeed() {
  ...
  seedRows(agents, seedAgents);
  seedRows(capabilityDefinitions, seedDefinitions);
  seedRows(agentCapabilities, seedAssemblies);
  seedRows(projects, seedProjects);
  seedRows(projectAgents, seedProjectAgentRows);
  seedRows(agentRuns, runs);

  const counts = {
    agents: db.select().from(agents).all().length,
    definitions: db.select().from(capabilityDefinitions).all().length,
    assemblies: db.select().from(agentCapabilities).all().length,
    projects: db.select().from(projects).all().length,
    projectAgents: db.select().from(projectAgents).all().length,
    runs: db.select().from(agentRuns).all().length,
  };
  return counts;
}

/** 清空全部业务表（reset 用；保留 migration 历史） */
